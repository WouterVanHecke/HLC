...
describe('Starting tests for tutorialApp', function() {
  ...
  it('website title should be tutorialApp', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tutorialApp');
    })
  });

  it('network-name should be tutorial@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorial@0.0.1.bna');
    });
  });

  it('navbar-brand should be tutorialApp',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorialApp');
    });
  });
